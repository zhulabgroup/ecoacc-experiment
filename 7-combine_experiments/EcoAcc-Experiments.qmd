...
### Changes in CTI over time between treatments
...
ok_scatter <- ggscatter(NPP_CTI_ok, x = "mean_ab_bio", y = "mean_CTI", 
                           add = "reg.line", conf.int = TRUE, 
                           cor.coef = TRUE, cor.method = "pearson",
                           xlab = "Biomass", ylab = "Oklahoma\nCTI")
```
...
### Shifts in CTI and CPI; seventh year for all experiments
...
ggarrange(CTI_jrgce_plot,CTI_phace_plot,CTI_teracon_plot,CTI_ok_plot,CTI_b4_cfc_plot,CTI_b4_hwrc_plot,
          ncol = 3, nrow=2)
```
...
### Stats: Which species show significant increases or decreases in abundance as a function of year?
# Perform regression analysis for each species
results_phace_year <- phace_spp_abun %>%
  group_by(species) %>%
  do(model = lm(avg_abun ~ year, data = .))
results_ok_year <- ok_spp_abun %>%
  group_by(species) %>%
  do(model = lm(avg_abun ~ year, data = .))
results_jrgce_year <- jrgce_spp_abun %>%
  filter(!(species == "Madia gracilis" | species == "Quercus lobata" | species == "Festuca DUMMY")) %>% # removing spp w/ NAs
  group_by(species) %>%
  do(model = lm(avg_abun ~ year, data = .))
results_tera_year <- tera_spp_abun %>%
  filter(!(species == "Total Planted Species")) %>%
  group_by(species) %>%
  do(model = lm(avg_abun ~ year, data = .))
results_b4_cfc_year <- b4_spp_abun_cfc %>%
  group_by(species) %>%
  do(model = lm(avg_abun ~ year, data = .))
results_b4_hwrc_year <- b4_spp_abun_hwrc %>%
  group_by(species) %>%
  do(model = lm(avg_abun ~ year, data = .))

# Extract coefficients and p-values
results_phace_summary_year <- results_phace_year %>%
  rowwise() %>%
  mutate(slope = coef(model)[["year"]],
         p_value = summary(model)$coefficients["year", "Pr(>|t|)"])
results_ok_summary_year <- results_ok_year %>%
  rowwise() %>%
  mutate(slope = coef(model)[["year"]],
         p_value = summary(model)$coefficients["year", "Pr(>|t|)"])
results_jrgce_summary_year <- results_jrgce_year %>%
  rowwise() %>%
  mutate(slope = coef(model)[["year"]],
         p_value = summary(model)$coefficients["year", "Pr(>|t|)"])
results_tera_summary_year <- results_tera_year %>%
  rowwise() %>%
  mutate(slope = coef(model)[["year"]],
         p_value = summary(model)$coefficients["year", "Pr(>|t|)"])
results_b4_summary_cfc_year <- results_b4_cfc_year %>%
  rowwise() %>%
  mutate(slope = coef(model)[["year"]],
         p_value = summary(model)$coefficients["year", "Pr(>|t|)"])
results_b4_summary_hwrc_year <- results_b4_hwrc_year %>%
  rowwise() %>%
  mutate(slope = coef(model)[["year"]],
         p_value = summary(model)$coefficients["year", "Pr(>|t|)"])
...
ggplot(ok_sig_spp, aes(x = mean_CTI, y = avg_abun)) +
  geom_smooth(method="lm") +
  geom_point() +
  theme_minimal() +
  labs(title = "Oklahoma: Mean CTI vs Abundance for Different Species",
       x = "Mean CTI",
       y = "Abundance") +
  #scale_color_manual(values = c("ambient" = "blue", "warmed" = "red")) +
  facet_wrap(~ species, scales = "free_y")
