# TITLE:          jrgce niche calculation for different gbif sales
# AUTHORS:        Kara Dobson
...
# Load in data
chelsa_uscan <- read.csv(" CHELSA_jrgce_uscan.csv")
chelsa_1000 <- read.csv(" CHELSA_jrgce_1000.csv")
chelsa_500 <- read.csv(" CHELSA_jrgce_500.csv")
...
write.csv(niche_est_uscan,paste(path_out,'jrgce_niche_uscan.csv'),row.names=F)
write.csv(niche_est_1000,paste(path_out,'jrgce_niche_1000.csv'),row.names=F)
write.csv(niche_est_500,paste(path_out,'jrgce_niche_500.csv'),row.names=F)
